...
install_packages() {
    log_info "安装必要的软件包..."
    apt install -y btop vnstat duf vim screen build-essential jq git libssl-dev unzip curl sudo wget ca-certificates bc neofetch cron
}
...
pull_docker_images() {
    ...
    docker build . --tag ghcr.io/lushdog/antnode:latest --build-arg VERSION=2025.12.1.1
}
